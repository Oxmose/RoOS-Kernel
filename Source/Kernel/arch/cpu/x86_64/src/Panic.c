...
/** @brief Stores the interrupt titles. */
static char* skpInterruptHeader[] =
{
  "Division by zero",
...
static void _PrintHeader(const S_VirtualCPU* kpVCpu, const bool kInterrupt)
{
  ...
  if (kpIntState->intId < 22)
  {
    _PanicPrintf(skpInterruptHeader[kpIntState->intId]);
  }
  else
  {
...
static void _PrintStackTrace(const S_VirtualCPU* kpVCpu)
{
  ...
  isMapped = MemoryIsMappedWithFlags(lastRBP,
                                     2 * sizeof(uintptr_t),
